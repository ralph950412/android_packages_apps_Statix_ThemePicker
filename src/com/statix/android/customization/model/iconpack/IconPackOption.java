...
public class IconPackOption implements CustomizationOption<IconPackOption> {
    ...
    public static final String[] OVERLAY_PACKAGES = {
        ANDROID_PACKAGE, SETTINGS_PACKAGE, SYSUI_PACKAGE
    };
    ...
    public void bindThumbnailTile(View view) {
        ...
        Drawable icon =
                mIcons.get(THUMBNAIL_ICON_POSITION).getConstantState().newDrawable().mutate();
        int colorFilter =
    ...
    public boolean isActive(CustomizationManager<IconPackOption> manager) {
        ...
        if (mIsDefault) {
            return overlayManager.getEnabledPackageName(SYSUI_PACKAGE, OVERLAY_CATEGORY_ICON_SYSUI)
                            == null
                    && overlayManager.getEnabledPackageName(
                                    SETTINGS_PACKAGE, OVERLAY_CATEGORY_ICON_SETTINGS)
                            == null
                    && overlayManager.getEnabledPackageName(
                                    ANDROID_PACKAGE, OVERLAY_CATEGORY_ICON_ANDROID)
                            == null;
        }
        for (Map.Entry<String, String> overlayEntry : getOverlayPackages().entrySet()) {
            if (overlayEntry.getValue() == null
                    || !overlayEntry
                            ...
                            .equals(
                                    overlayManager.getEnabledPackageName(
                                            determinePackage(overlayEntry.getKey()),
                                            overlayEntry.getKey()))) {
                return false;
            }
        }
    ...
    public void bindPreview(ViewGroup container) {
        ...
        if (cardBody.getChildCount() == 0) {
            LayoutInflater.from(container.getContext())
                    .inflate(R.layout.preview_card_icon_content, cardBody, true);
        }
        for (int i = 0; i < mIconIds.length && i < mIcons.size(); i++) {
            ((ImageView) container.findViewById(mIconIds[i])).setImageDrawable(mIcons.get(i));
        }
    }
